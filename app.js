...
function iniciarApp() {
    // --- IMPORTAÇÃO CORRIGIDA PARA O TEU CSV ---
    document.getElementById('csvInput').onchange = (e) => {
        ...
        reader.onload = (event) => {
            ...
            lines.forEach((line, index) => {
                ...
                // Força a separação por ponto e vírgula
                const cols = line.split(';');
                if (cols.length < 6) return;

                // Extração direta baseada no teu ficheiro real:
                // 0:Filial, 1:Pedido, 2:CodForn, 3:NomeForn, 4:Valor, 5:CC, 6:Vencimento
                const valorBruto = cols[4]?.trim();
                ...
                push(contasRef, {
                    ...
                    valor: valorNumerico,
                    centroCusto: cols[5]?.trim() || "000000",
            ...
            alert(`✅ ${contador} lançamentos importados para o mês atual!`);
            e.target.value = ""; 
        };
        // ISO-8859-1 trata os caracteres especiais do Excel (ç, ã) corretamente
...
function ativarEdicaoInLine() {
    document.querySelectorAll('.editable').forEach(el => {
        el.onclick = function() {
            ...
            let val = this.innerText.replace('R$ ', '').trim();
